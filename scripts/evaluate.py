...
from barcgp.prediction.trajectory_predictor import ConstantVelocityPredictor, ConstantAngularVelocityPredictor, GPPredictor, NoPredictor, NLMPCPredictor, MPCPredictor, CAMPCCPredictor
...
def runSimulation(dt, t, N, name, predictor, scenario, id, offset=0):
    ...
    lencar = ego_dynamics_config.wheel_dist_front + ego_dynamics_config.wheel_dist_rear
    widthcar = ego_dynamics_config.bump_dist_center * 1.5
    # lencar = 0.37
    # widthcar = 0.195
    r_d = widthcar # / 1.8
    dl = lencar * 0.9
    ...
    slack_l_hard = 0.4 * lencar
    slack_w_hard = 0.4 * widthcar
    ...
    a = (lencar / np.sqrt(2) + r_d + slack_l)
    b = (widthcar / np.sqrt(2) + r_d + slack_w)
            ...
            if predictor:
                ...
                if ego_pred.x is not None:
                    try:
                        ...
                        ey_pred = []
                        ep_pred = []
                        for _x, _y, _p in zip(ego_pred.x[1:], ego_pred.y[1:], ego_pred.psi[1:]):
                            _s, _ey, _ep = track_obj.global_to_local((_x, _y, _p))
                            if s_prev is None:
                                s_prev = copy.copy(_s)
                            ...
                            ey_pred.append(_ey)
                            ep_pred.append(_ep)
                        s_pred.append(s_pred[-1])
                        ey_pred.append(ey_pred[-1])
                        ep_pred.append(ep_pred[-1])
                        ego_pred.s = array.array('d', s_pred)
                        ego_pred.x_tran = array.array('d', ey_pred)
                        ego_pred.e_psi = array.array('d', ep_pred)
                    except:
                        ego_pred = ego_pred_prev
                    ego_pred_prev = copy.deepcopy(ego_pred)

                    if ego_sim_state.p.s - tar_sim_state.p.s > track_obj.track_length/2:
                        tar_sim_state.p.s += track_obj.track_length

                    # print(ego_sim_state.p.s, tar_sim_state.p.s, ego_pred.s)
                    opp_pred = predictor.get_prediction(ego_sim_state, tar_sim_state, ego_pred, tar_prediction)
